...
* 1.5 캐쉬
    ...
    - 디스크는 크기 떄문에, 찾는 속도가 느린 방면, 메인메모리는 작고, 빨리 찾음
        - 용량으로 예를 들면, 확실하게 알수 있다.

    - 메인메모리를 쓰는이유: 디스크에 접근하기 쉬워서!

    - 계층구조? 전지적 cpu 시점 : 최대한 디스크에 가지 않게 하여, 속도를 올린다.
...
* 1.7
    ...
    - 운영체제 쉘
        - 커널 (메모리 자료구조 --> 운영체제 함수를 담고 있음)
            - 운영체제 시점에서 프로세스가 존재하는데, 그 접근을 막는다
        - 유저
            - 커널에게 명령을 내릴 수 있음

    - 쓰레드
        - 작업의 흐름: 이전 상테에 대한 정보를 가져와 진행

    - 다중 쓰레드....서버 게임 C 등등에서 다나옴
